...
export class WishlistClient {
    ...
    cloneWishlist(wishlistId: string | undefined, newName: string | null | undefined): Promise<void> {
        let url_ = this.baseUrl + "/Wishlist/cloneWishlist?";
        if (wishlistId === null)
            throw new Error("The parameter 'wishlistId' cannot be null.");
        else if (wishlistId !== undefined)
            url_ += "wishlistId=" + encodeURIComponent("" + wishlistId) + "&";
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processCloneWishlist(_response);
        });
    }

    protected processCloneWishlist(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    removeWishlist(wishlistId: string | undefined): Promise<void> {
        let url_ = this.baseUrl + "/Wishlist/removeWishlist?";
        if (wishlistId === null)
            throw new Error("The parameter 'wishlistId' cannot be null.");
        else if (wishlistId !== undefined)
            url_ += "wishlistId=" + encodeURIComponent("" + wishlistId) + "&";
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processRemoveWishlist(_response);
        });
    }

    protected processRemoveWishlist(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
